...
namespace NinjaFramework {
    namespace JSON {
        ...
        void Document::SetRoot(std::shared_ptr<Iterable> &root) {
            _root = root;
        }
        ...
        void Document::Write(std::wstring &string) {
            string = _root->JSONString();
        }
        
        void Document::Write(std::wostream &stream) {
            stream << _root->JSONString();
        }

        std::shared_ptr<Document> Document::Parse(std::wistream &stream) {
            JSONParseStream *json_stream = new JSONParseStream(&stream);
            return ParseDocument(json_stream);
        }
                ...
                switch (character) {
                    case '{':
                        if (root != nullptr) {
                            ...
                            PARSE_ERROR("JSON parse error: Multiple roots detected", source);
                        }
                    ...
                    WHITESPACE_CASES
                        break;
                    default:
                    PARSE_ERROR("JSON parse error: Invalid character at root level", source);
                }
                character = source->get();
            }
            if (root == nullptr) {
                PARSE_ERROR("JSON parse error: Empty document", source);
            }
        ...
        std::shared_ptr<Object> Document::ParseObject(JSONParseStream *source) {
                ...
                switch (character) {
                    case '"':
                        if (require_delimiter) {
                            PARSE_ERROR("JSON parse error: Missing comma", source);
                        }
                    ...
                    case ':': {
                        ...
                        if (value == nullptr) {
                            PARSE_ERROR("JSON parse error: Value expected", source);
                        }
                        obj->Add(key, value);
                        key = L"";
                    ...
                    case ',':
                        if (!require_delimiter) {
                            PARSE_ERROR("JSON parse error: Missing key:value pair", source);
                        }
                    ...
                    case '}':
                        // End of object hit
                        if (key.length() > 0) {
                            PARSE_ERROR("JSON parse error: Key has no associated value", source);
                        }
                        done = true;
                        break;
                    default:
            ...
            if (!done) {
                PARSE_ERROR("JSON parse error: Object is not closed", source);
            }
            return obj;
        }
        ...
        std::shared_ptr<Array> Document::ParseArray(JSONParseStream *source) {
            ...
            if (!done) {
                PARSE_ERROR("JSON parse error: Array is not closed", source);
            }
            return array;
        }
        ...
        std::shared_ptr<String> Document::ParseString(JSONParseStream *source) {
            ...
            if (!done) {
                PARSE_ERROR("JSON parse error: String not terminated", source);
            }
            return value;
        }
        ...
        std::shared_ptr<Number> Document::ParseNumber(JSONParseStream *source) {
            ...
            if (!done) {
                PARSE_ERROR("JSON parse error: Invalid JSON", source);
            }
            return value;
        }
            ...
            while (!done && !source->eof()) {
                ...
                if (character >= 'a' && character <= 'z') {
                    ...
                    if (string == L"true") {
                        ...
                        PARSE_ERROR("JSON parse error: Invalid string literal", source);
                    }
                }
                if (!done) {
                    character = source->get();
